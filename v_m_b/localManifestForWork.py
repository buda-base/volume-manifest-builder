...
import ImageGroupResolver
image_group_resolver: ImageGroupResolver = None
...
def prolog() -> VMBArgs:
    ...
    global shell_logger, image_group_resolver
    ...
    image_group_resolver = ImageGroupResolver(args.source_container, args.image_classifier)
...
def parse_args(arg_namespace: object) -> None:
    ...
    from DBApps.BuildStatusUpdater import mustExistDirectory
...
def manifestForList(args: VMBArgs):
    ...
    source_file: str = args.sourceFile
...
def manifestForVolume(client, workRID, vi, csvwriter):
    ...
    igPath: Path = get_s3_folder_prefix(workRID, vi.imageGroupID)
...
def manifestExists(client, s3folderPrefix):
    ...
    key = s3folderPrefix + 'dimensions.json'
    try:
        client.head_object(Bucket=S3_SOURCE_BUCKET, Key=key)
        return True
...
def fillData(transfer, s3imageKey, csvwriter, imgdata, workRID, imageGroupID):
    ...
    filename = io.BytesIO()
    try:
        transfer.download_file(S3_SOURCE_BUCKET, s3imageKey, filename,
...
def _generateManifest(client, s3folderPrefix, imageList, csvwriter, workRID, imageGroupID):
    """
...
async def generateManifest(igContainer: Path, imageList, csvwriter, workRID, imageGroupID):
    """
    this actually generates the manifest. See example in the repo. The example corresponds to W22084, image group I0886.
    :param igContainer: path of parent of image group
    :param s3folderPrefix:
    :param imageList: list of image names
    :param csvwriter:
    :param workRID:
